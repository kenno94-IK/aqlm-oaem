...
class AQEngine(nn.Module):
    ...
    def quantize(self, *, args: Namespace, verbose: bool = True) -> QuantizedWeight:
        ...
        # Pass XTX for OA-EM refinement
        oa_em_rounds = getattr(args, 'oa_em_rounds', 0)
        pass_xtx = oa_em_rounds > 0
        ...
        self.quantized_weight = QuantizedWeight(
            ...
            verbose=True,
            XTX=self.XTX.float() if pass_xtx else None,
            oa_em_rounds=oa_em_rounds,
            oa_em_lr=getattr(args, 'oa_em_lr', 1e-4),
            oa_em_steps=getattr(args, 'oa_em_steps', 100),
        )
