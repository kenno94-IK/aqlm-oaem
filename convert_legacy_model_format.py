...
def load_quantized_model_from_fdsp_checkpoint(base_model_name: str, fsdp_checkpoint_path: str, **kwargs):
    ...
    non_quant_states = torch.load(non_quant_path, weights_only=False)
    ...
    for module_name, module in original_model.named_modules():
        ...
        quantized_weight = torch.load(state_path, map_location="cpu", weights_only=False)
        bias_param = module.bias if module.bias is not None else None
        quantized_linear = QuantizedLinear(quantized_weight, bias=bias_param)
...
def main():
    ...
    if args.p_finetuned_state_dict is not None:
        state_dict = torch.load(args.p_finetuned_state_dict, map_location="cpu", weights_only=False)
    ...
    # Copy args.pt from original quantized model so convert_to_hf.py can read metadata
    import shutil
    args_pt_src = os.path.join(args.quantized_model, "args.pt")
    args_pt_dst = os.path.join(args.save, "args.pt")
    if os.path.exists(args_pt_src) and not os.path.exists(args_pt_dst):
        shutil.copy2(args_pt_src, args_pt_dst)
