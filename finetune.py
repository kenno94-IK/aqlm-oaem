...
def add_finetuning_args(parser: argparse.ArgumentParser):
    ...
    parser.add_argument("--save_final_model", action="store_true", help="Save model at end of training to final_model/")
    parser.add_argument(
        "--on_save",
        type=str,
        default=None,
...
def add_data_args(parser: argparse.ArgumentParser):
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Specific data file(s) to download, e.g. 'en/c4-train.00000-of-01024.json.gz'",
    )
    parser.add_argument(
        "--cache_dir",
        type=str,
...
def prepare_training_dataset(args: argparse.Namespace, tokenizer: transformers.PreTrainedTokenizer) -> datasets.Dataset:
    if os.path.exists(args.dataset_name):
        ...
        if args.data_files is not None:
            load_kwargs["data_files"] = {args.split.split("[")[0]: args.data_files}
...
def load_student_model(
    ...
    # Untie lm_head from embed_tokens to avoid FSDP writeback shape mismatch during backward.
    # When weights are tied, FSDP's use_orig_params=True fails with "Cannot writeback when the gradient shape changes".
    if getattr(student_model.config, "tie_word_embeddings", False):
        lm_head = student_model.get_output_embeddings()
        lm_head.weight = nn.Parameter(lm_head.weight.clone())
        student_model.config.tie_word_embeddings = False
        if torch.distributed.get_rank() == 0:
            print("Untied lm_head from embed_tokens for FSDP compatibility")
...
def load_training_state(
    ...
    rank = torch.distributed.get_rank()
    checkpoint_file = os.path.join(args.save, f"quantized_model_state_dict_rank{rank}.pt") if args.save else None
    if args.save is None or not os.path.exists(args.save) or not os.path.exists(checkpoint_file):
        if args.save is not None and rank == 0:
            print(f"No checkpoint found at {args.save}, starting from scratch")
    else:
        with FullyShardedDataParallel.state_dict_type(quantized_model, StateDictType.LOCAL_STATE_DICT):
            ...
            loaded_state_dict = torch.load(os.path.join(args.save, f"quantized_model_state_dict_rank{rank}.pt"), weights_only=False)
            with torch.no_grad():
        ...
        optimizer.load_state_dict(
            torch.load(os.path.join(args.save, f"optimizer_state_dict_rank{rank}.pt"), map_location="cpu", weights_only=False)
        )
        metadata.update(torch.load(os.path.join(args.save, "metadata.pt"), weights_only=False))
...
def save_model(args: argparse.Namespace, student_model: FullyShardedDataParallel, optimizer: torch.optim.Optimizer,
    ...
    if isinstance(optimizer, StraightThroughAdamW):
        save_pv_model(args, student_model, optimizer, subdir=subdir)
    else:
...
def save_pv_model(
    ...
    output_path = os.path.join(args.save, subdir)
    ...
    if rank == 0:
        print(f"Saved model shards to {output_path}")
...
def main():
    ...
    for current_epoch in range(args.max_epochs):
        ...
        for dataset_name, perplexity in perplexity_scores.items():
            metadata[f"perplexity_{dataset_name}"] = perplexity
        if rank == 0:
            print(f"End of epoch {metadata['current_epoch']} eval: "
                  + ", ".join(f"{k}={v:.4f}" for k, v in perplexity_scores.items()))
        metric_name = metadata["early_stop_on"]
        if perplexity_scores[metric_name] < metadata["best_eval_perplexity"]:
            if rank == 0:
                print(f"New best perplexity ({metric_name}) = {perplexity_scores[metric_name]:.9f}")
    ...
    if getattr(args, 'save_final_model', False) and args.save is not None:
        rank = torch.distributed.get_rank()
        if rank == 0:
            print("Saving final model state...")
        save_model(args, student_model, optimizer, subdir="final_model")
        if args.eval_datasets is not None:
            perplexity_scores = compute_validation_perplexities(args, student_model, eval_datasets)
            if rank == 0:
                for dataset_name, perplexity in perplexity_scores.items():
                    print(f"Final {dataset_name} perplexity: {perplexity:.9f}")
