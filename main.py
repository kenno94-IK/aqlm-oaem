...
def get_inps(
    ...
    # LlamaModel in transformers>=4.38 has a top-level rotary_emb that must be on the same device
    ...
    for layer_index in range(len(layers)):
        ...
        if args.resume:
            ...
            if os.path.exists(layer_save_path):
                ...
                layer = torch.load(layer_save_path, map_location=args.devices[0], weights_only=False)
                loaded_layer = True
...
def main():
    ...
    parser.add_argument(
        ...
        type=int,
        default=0,
        help="Number of OA-EM (output-aware EM) refinement rounds after k-means init. 0=disabled.",
    )
    parser.add_argument(
        "--oa_em_lr",
        type=float,
        default=1e-4,
        help="Learning rate for OA-EM centroid optimization",
    )
    parser.add_argument(
        "--oa_em_steps",
        type=int,
        default=100,
        help="Number of Adam steps per OA-EM M-step",
    )
    parser.add_argument(
        "--init_max_iter",
        type=int,
