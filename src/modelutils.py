...
def get_model(
    ...
    with suspend_nn_inits():
        model = AutoModelForCausalLM.from_pretrained(
            ...
            local_files_only=os.environ.get("AQLM_LOCAL_ONLY", "0") == "1",
            **model_kwargs,
        )
        if load_quantized:
...
def read_quant_weight_from_file(load_path, block_i, layer_name, device):
    return torch.load(load_path + "/" + str(block_i) + "/" + layer_name, map_location=device, weights_only=False)
...
def load_dequantized_model(model, load_path):
    ...
    for layer_index in range(len(layers)):
        ...
        quant_layer = torch.load(os.path.join(load_path, str(layer_index) + ".pth"), map_location="cpu", weights_only=False)
    ...
    model.load_state_dict(torch.load(os.path.join(load_path, "not_quantized_weights.pt"), weights_only=False), strict=False)
    return model
