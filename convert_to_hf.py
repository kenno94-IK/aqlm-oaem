...
def get_converted_state_dict(config, nbits, in_path):
    state_dict = {}
    ...
    for key, value in torch.load(os.path.join(in_path, "not_quantized_weights.pt"), weights_only=False).items():
...
def get_metadata(in_path: os.PathLike) -> dict:
    quant_args = torch.load(os.path.join(in_path, "args.pt"), weights_only=False)
    return {
...
def update_config(config_dict, aqlm_metadata, linear_weights_not_to_quantize):
